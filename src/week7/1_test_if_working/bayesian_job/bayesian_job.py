...
def spring_trial(
    ...
    m_nodes = rng.lognormal(mean=mu, sigma=m_spread, size=num_nodes)
    m_diag = np.repeat(m_nodes, dims)
    m_inv_diag_cp = cp.asarray(1.0 / m_diag)

    mu = np.log(c_val) - (c_spread**2 / 2)
    c_nodes = rng.lognormal(mean=mu, sigma=c_spread, size=num_nodes)
    c_diag_cp = cp.asarray(np.repeat(c_nodes, dims))

    mu = np.log(k_wall_val) - (k_wall_spread**2 / 2)
    k_wall_vals = rng.lognormal(
    ...
    displacement_cp, velocity_cp = run_simulation(
        ...
        dt=dt,
        m_inv_diag_cp=m_inv_diag_cp,
        c_diag_cp=c_diag_cp,
        U_cp=U_cp * input_force,
        initial_pos_cp=nodes_pos_cp,
        connections_list_cp=connections_list_cp,
        k_vals_cp=k_vals_cp,
        rest_lens_cp=rest_lens_cp,
        wall_nodes=wall_nodes,
    )

    movement_disp = displacement_cp[:, movement_idx]
    movement_vel = velocity_cp[:, movement_idx]
    ...
    if has_overlap or has_invalid:
        first_overlap = int(cp.argmax(overlap_mask)) if has_overlap else len(X)
        first_invalid = int(cp.argmax(invalid_steps_mask)) if has_invalid else len(X)
        first_failure = min(
            len(X),
            int(cp.where(overlap_mask)[0][0]) if has_overlap else len(X),
